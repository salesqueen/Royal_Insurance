<?php 
    ...
    if($constraint==""){
        $pending_policy_result_set=$user->read_selective_policy("WHERE comission_percentage=0 AND agent_id=".$_SESSION['id']);
    }else{
        $pending_policy_result_set=$user->read_selective_policy("WHERE comission_percentage=0 AND agent_id=".$_SESSION['id']." ".$constraint);
    }
